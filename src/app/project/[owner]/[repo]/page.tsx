...
import ProjectContributorsSlide from "./slides/ProjectContributorsSlide";
import ProjectReleasesSlide from "./slides/ProjectReleasesSlide";
...
export interface ProjectData {
  ...
  repository: {
    ...
    languages: Array<{ name: string; color: string; percentage: string }>;
    topics: string[];
    license: string | null;
  };
  stats: {
    stars: { total: number; gained2025: number };
    forks: { total: number; gained2025: number };
    watchers: number;
  ...
  activity: {
    ...
    weeklyCommits: Array<{ week: string; count: number }>;
  };
  personality: {
    archetype: string;
    title: string;
    ...
    tagline: string;
    description: string;
    traits: Array<{ name: string; description: string; icon: string }>;
  };
}
...
export default function ProjectWrappedPage() {
  ...
  const slides = [
    { component: ProjectIntroSlide, duration: 6000 },
    { component: ProjectStatsSlide, duration: 8000 },
    { component: ProjectContributorsSlide, duration: 7000 },
    { component: ProjectReleasesSlide, duration: 7000 },
    { component: ProjectActivitySlide, duration: 8000 },
    { component: ProjectImpactSlide, duration: 7000 },
    { component: ProjectSummarySlide, duration: 0 },
  ];
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const response = await fetch(`/api/project?owner=${encodeURIComponent(owner)}&name=${encodeURIComponent(repo)}`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0a0f0d]">
        <div className="text-center">
          ...
          />
          <p className="text-gray-400">Generating project wrapped...</p>
          <p className="text-gray-600 text-sm mt-2">Fetching data for {owner}/{repo}</p>
        </div>
      </div>
    );
  }
